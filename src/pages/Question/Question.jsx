  ...
  const handleStartExam = async () => {
    try {
      ...
      if (response.status === 200) {
        ...
        setError(
          response.error || "Exam has not started yet. Please check back later."
        );
        alert(
          response.error || "Exam has not started yet. Please check back later."
        );
    ...
    <AnimateProvider className="max-w-3xl mx-auto">
      ...
      {userInfo && (
        <div className="mb-5 flex">
          <div className="info">
            <h2 className="text-4xl font-semibold text-orange-700 mb-3">
              Welcome{" "}
            </h2>
            <h3 className="text-gray-500 text-xl font-semibold">
              Symbol Number: {userInfo.data.symbol_number}
            </h3>
            <h3 className="text-gray-500 text-xl font-semibold">
            ...
            <h3 className="text-gray-500 text-xl font-semibold">
              Program ID: {userInfo.data.program.name}
            </h3>
            <h3 className="text-gray-500 text-xl font-semibold">
              Level ID: {userInfo.data.level.name}
            </h3>
            <h3 className="text-gray-500 text-xl font-semibold">
              Exam Time: {formatTime(userInfo.data.duration)}
            </h3>
          </div>
